...
const OrderForm = () => {
  ...
  const handleCheckboxChange = (event) => {
    ...
    // console.log('Checkbox change detected:'.itemId, event.target.checked);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Log the packages and items only when the order is placed
        ...
        <div>
          ...
          <ul>
            {packages.map((pkg, index) => (
              <li key={index}>
                Package {index + 1}: {pkg.items.join(", ")}
              </li>
            ))}
          </ul>
        </div>
